...
export default defineNuxtRouteMiddleware((to, from) => {
  ...
  if (!token.value && to.meta.auth === true) return navigateTo($localePath('/auth/login'))

  const authorities = data.value?.authorities || []

  if (token.value && to.meta.unauthenticatedOnly === true) {
    if (crossValArr(authorities, [ROLES.ROLE_ADMIN])) return navigateTo($localePath('/admin/locations'))
    if (crossValArr(authorities, [ROLES.ROLE_MANAGER])) return navigateTo($localePath('/admin/checkProduct'))
    if (authorities.includes(ROLES.ROLE_USER)) return navigateTo($localePath('/user'))
