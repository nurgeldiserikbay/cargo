...
class HttpFactory {
  ...
  async POST<DataT>(url: string, data?: object, extras = {}): HttpFactoryResult<DataT, ErrorT> {
    return await this.CALL<DataT>('POST', url, data, extras)
  }

  async PUT<DataT>(url: string, data?: object, extras = {}): HttpFactoryResult<DataT, ErrorT> {
    return await this.CALL<DataT>('PUT', url, data, extras)
  }

  async DELETE<DataT>(url: string, extras = {}): HttpFactoryResult<DataT, ErrorT> {
