	...
	<v-card class="!min-h-full">
		<v-layout class="!min-h-full">
			...
			<v-app-bar color="primary" prominent>
				...
				<v-toolbar-title v-if="getCurrentRoute">{{
					$t(getCurrentRoute.text)
				}}</v-toolbar-title>
			...
			>
				<v-list>
					<v-list-item
						...
						:key="item.to"
						:active="route.path === localePath(item.to)"
...
const getMenuList = computed(() => {
	return ADMIN_MENUS.filter((m) =>
		crossValArr(m.roles, data.value?.authorities || []),
	)
})
const getCurrentRoute = computed(() =>
	getMenuList.value.find((r) => route.path === localePath(r.to)),
)
