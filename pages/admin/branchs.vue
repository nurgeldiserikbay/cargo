<template>
  <v-container>
    <CreateBranch :selectedBranch="selectedBranch" @clear="clear" @added="addBranch" />
    ...
    <v-data-iterator :items="list" :page="curPage" :items-per-page="perPage">
      <template v-slot:default="{ items }">
        <v-list :variant="'elevated'">
          <v-list-item v-for="(item, i) in items" :key="i" :variant="'elevated'" class="mb-4 cursor-pointer"
            @click="select(item.raw)">
            <v-row>
              <v-col cols="6" :align-self="'center'" class="border !border-r-(!2px solid color1)">
              ...
              <v-col class="flex justify-end gap-2">
                <v-btn variant="plain" color="primary" :icon="true">
                  <v-tooltip activator="parent" branch="top">{{ $t(`commands.select`) }}</v-tooltip>
                  <v-icon icon="mdi mdi-button-pointer" size="large"></v-icon>
                </v-btn>
                <v-btn variant="plain" color="error" :icon="true" class="cursor-pointer" @click.stop="remove(item.raw)">
                  <v-tooltip activator="parent" branch="top">{{ $t(`commands.clear`) }}</v-tooltip>
...
const getCityName = computed(() => (id: number) => locationStore.getCityById(id)?.name || '')
