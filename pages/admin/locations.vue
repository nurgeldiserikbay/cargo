<template>
  <v-container>
    <CreateLocation :selectedLocation="selectedLocation" @clear="clear" @added="addCity" />
          ...
          <v-list-item v-for="(item, i) in items" :key="i" :variant="'elevated'" class="mb-4 cursor-pointer"
            ...
            <v-row>
              ...
              <v-col class="flex justify-end gap-2">
                ...
                <v-btn variant="plain" color="error" :icon="true" class="cursor-pointer"
                  @click.stop="remove(item.raw)">
                  <v-tooltip activator="parent" location="top">{{ $t(`commands.clear`) }}</v-tooltip>
